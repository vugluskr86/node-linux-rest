...
D3.Chart = Backbone.View.extend({
    ...
    initialize: function(options) {
        ...
        _.defaults(
            this,                       // Options for this instance
    ...
    getX: function(d) { return this._getDatumValue(d, this.xAttr); },

    // Get the y value for a datum
    getY: function(d) { return this._getDatumValue(d, this.yAttr); },
...
D3.Bar = D3.Chart.extend({
    ...
    getXScale: function() {
        return d3.scale.ordinal()
            .rangeRoundBands([0, this.width], this.barPadding)
            .domain(this.collection.pluck(this.xAttr));
    },

    getYScale: function() {
        return d3.scale[this.yScale]()
            .rangeRound([this.height, 0])
            .domain([
                0,
                this.getLinearExtent(this.yAttr, 'max')
            ])
            .nice();
    }
...
D3.Line = D3.Chart.extend({
    ...
    renderData: function() {
        ...
        var line = d3.svg.line()
            ...
            .defined(function(d) { return d && this.yValid(d[this.yAttr]); })
            .x(function(d) { return x(d[this.xAttr]); })
            .y(function(d) { return y(d[this.yAttr]); });
        ...
        series.append('path')
            .attr('class', 'line')
            .attr('d', function(series) { return line(series.get(this.valuesAttr)); })
            .style('stroke', function(series, i) {
                return series.get(this.colorAttr) || this.colorScale(i);
            });
    },

    getXScale: function() {
        return d3.scale[this.xScale]()
            .rangeRound([0, this.width])
            .domain(this.getLinearExtent(this.xAttr));
    },
    ...
    getLinearExtent: function(attr, minmax) {
        ...
        var seriesExtents = this.collection.map(function(series) {
            var values = series.get(this.valuesAttr);
...
D3.CountryMap = D3.Chart.extend({
        ...
        this.svg.append('g')
            ...
            .selectAll('path')
                ...
                .enter().append('path')
                    ...
                    .style('stroke-width', this.getStrokeWidth(this.strokeWidth));
    },
    ...
    getCountryTitle: function(f) {
        ...
        if (d && this.yValid(this.getY(d))) {
            title += ': ' + this.yFormat(this.getY(d));
        }
        return title;
    },
    ...
    getMapTransform: function() {
        ...
        var b = this.getDataBounds(this.autoZoom ? this.collection : null),
    ...
    getMapScale: function() {
        ...
        var zoomFactor = this.zoomFactor,
            b = this.getDataBounds(this.autoZoom ? this.collection : null),
            w = this.width,
            h = this.height;
    ...
    getDataBounds: function(data) {
        ...
        _(data).each(function(d) {
            var feature = _(this.countryData.features).find(function(f) {
        ...
        var proj = this.scales.x.projection(),
            crop = [proj(this.crop[0]), proj(this.crop[1])];
